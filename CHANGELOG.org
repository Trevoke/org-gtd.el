...
* 3.1.0 (unstable)
** WIP buffer is now a major mode derived from org-mode
No action is currently required. This does mean we make obsolete ~org-gtd-clarify-map~ in favor of ~org-gtd-wip-mode-map~.
This handles #178 even though technically that is incorrect user code, but since no one thinks of org-mode buffers as being disconnected from files, this is probably a good idea anyway, and it gives me a clean place for additional setup in the WIP buffers.
