...
*** View DSL: =property= filter
- The view DSL now honors =(property . (("PROP" . "VALUE")))= to include only
  items whose org property =PROP= equals =VALUE=.  Multiple pairs are combined
  with AND logic (e.g. =(property . (("ENERGY" . "high") ("CONTEXT" . "office")))=),
  and the filter composes with every other view filter.  Malformed specs signal
  a =user-error= at build time.  Previously the key was accepted but silently
  ignored.
...
*** Someday/maybe review is now resumable
Quitting a someday/maybe review mid-way and returning later resumes that
list where you left off.  Each list keeps an independent resume point, so
reviewing one selection no longer disturbs your place in another.
