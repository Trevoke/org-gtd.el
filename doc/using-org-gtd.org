...
Open it with ~M-x org-gtd-view-manager~, or press =v= in the command center. If you have no saved views yet, this opens the builder directly so you can make your first one. Otherwise it prompts you to pick a saved view by name through completing-read -- each name is annotated with a short summary of what it filters, or, for a multi-section view, a summary of its sections -- and then opens a small action menu for the view you picked:

- =o= open the view as a live agenda;
- =e= edit it in the builder;
- =n= create a new view (opens the builder on a blank view);
- =c= copy it (opens the builder on a duplicate named "<name> copy");
- =d= delete it, after confirmation;
- =q= quit.

Because you pick by name, the manager stays comfortable no matter how many views you accumulate: type a few characters to narrow the list instead of scrolling through it.
...
*Recalling a saved view.* Use ~M-x org-gtd-view-run~ to recall any saved view by name through the same annotated completing-read, and org-gtd renders it as an agenda. This is handy to bind to a key or call from your own commands.
