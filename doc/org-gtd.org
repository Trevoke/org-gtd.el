...
~M-x org-gtd-graph-view~

*Requires*: Point must be on a project heading (not a task)
...
| Key   | Command          | Status                                                       |
|-------+------------------+--------------------------------------------------------------|
| z     | Zoom             | Not implemented (filter/focus features)                      |
| u     | Undo             | Not implemented (undo last graph change)                     |
| C-r   | Redo             | Not implemented (redo undone change)                         |
...
Open the stuck project in graph view: ~M-x org-gtd-graph-view~
...
The graph view supports comprehensive dependency management through keyboard commands. Press ~?~ in graph view to see the full command menu.

**** Dependency Operations

*Available commands*:
- ~a~: Add dependency between two tasks (supports convergence/diamond patterns)
- ~d~: Remove a dependency between two tasks
- ~x~: Clear all relationships from selected task
- ~i~: Show all relationships for selected task

**** Adding Tasks

*Available commands*:
- ~a c~: Add child task (depends on current task)
- ~a r~: Add root task (no dependencies)
- ~a b~: Add blocker task (blocks current task and becomes root)

**** Operations Not in Graph View

Some operations are standard org-mode functionality, not graph-specific:
...
*Deleting tasks*: Mark task DONE or CNCL in the org file. Completed tasks still show in graph view (as green boxes) to maintain project history.

**** When to Use Text-Based Commands
...
*See also*: [[#refiling-v4][Refiling section]] for complete documentation on creating refile targets and understanding the refile system.
...
*Purpose*: Marks a heading as a refile target for org-gtd's optional refile system (see [[#refiling-v4][Refiling section]])
...
4. **Emergency recovery from data corruption**
   ...
   - After manual edits, run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~
