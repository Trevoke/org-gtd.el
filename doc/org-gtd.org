...
By default (~org-gtd-refile-to-any-target~ is ~t~), org-gtd automatically refiles to the first target found without prompting. If you have multiple refile targets and want to choose between them:

#+begin_src elisp
(setq org-gtd-refile-to-any-target nil)
#+end_src

With this setting, org-gtd will always prompt you to select from available targets.
