...
- ~type~ :: project, next-action, delegated, calendar, incubated, habit, stuck-project, completed-project
...
**** Raw Timestamp: ~(timestamp . VALUE)~

Filter directly by ~ORG_GTD_TIMESTAMP~ property. Use ~when~ instead when filtering by type.
