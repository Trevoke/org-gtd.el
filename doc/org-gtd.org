...
#+SUBTITLE: for version 4.0.0beta1
#+TEXINFO_DEFFN: t
...
#+TEXINFO: @noindent
This manual is for Org GTD version 4.0.0beta1
...
- ~a s~ :: Add a successor (a new task that depends on the selected task)
- ~a b~ :: Add a blocker (a new task that the selected task depends on)

*The key insight:* ~a s~ and ~a b~ let you connect ONE new task to MULTIPLE existing tasks using a checkbox menu. This enables complex patterns like convergence (diamond).
...
2. ~a r~ → "Find place to get MRI" (root task 1)
3. ~a r~ → "Make orthopedist appointment" (root task 2 - now parallel!)
4. Select "Find place to get MRI"
5. ~a s~ → "Make MRI appointment" → check only "Find MRI" → RET
...
You could also build this using ~a b~ (add blocker):

1. Create "Get diagnosis" first as a root task (~a r~)
2. Select "Get diagnosis"
3. ~a b~ → "Make MRI appointment" → check "Get diagnosis" → RET
4. Still on "Get diagnosis", ~a b~ → "Make orthopedist appointment" → check "Get diagnosis" → RET
...
3. ~a s~ → "Implement features" → check "List features" → RET
4. Go back to "List features"
5. ~a s~ → "Write documentation" → check "List features" → RET
...
2. ~a s~ → "Update code" → check "Get Beta Feedback" → RET
3. Select "Get Beta Feedback" again
4. ~a s~ → "Update docs" → check "Get Beta Feedback" → RET
...
1. *Parallel branches* from List Features (using ~a s~ twice from same parent)
2. *Converging paths* at Get Beta Feedback (using multi-select checkbox)
3. *Parallel updates* after feedback (using ~a s~ twice)
...
*How to build*: ~a r~ for A, select A, ~a s~ for B (check A), select B, ~a s~ for C (check B).
...
*How to build*: ~a r~ for A, select A, then ~a s~ three times (B, C, D - each checking only A).
...
*How to build*: ~a r~ for A and B. Select A, ~a s~ for C, then check BOTH A and B in the checkbox menu.
...
*How to build*:
1. ~a r~ for A
2. Select A, ~a s~ for B (check A)
3. Select A again, ~a s~ for C (check A)
4. Select B, ~a s~ for D, then check BOTH B and C
...
| Key   | Command              | Description                                              |
|-------+----------------------+----------------------------------------------------------|
| a r   | Add root task        | Create task with no dependencies (starts immediately)    |
| a s   | Add successor        | Create task that the selected task must complete first   |
| a b   | Add blocker          | Create task that must complete before selected task      |

*Understanding successors and blockers:*

These two commands handle all dependency relationships:

- *Add successor* (~a s~): "What comes after this task?" Creates a new task that depends on the selected task. The selected task blocks the new task.

- *Add blocker* (~a b~): "What must happen first?" Creates a new task that the selected task depends on. The new task blocks the selected task.
...
*Modify Operations (~m~ prefix):*

| Key   | Command              | Description                                              |
|-------+----------------------+----------------------------------------------------------|
| m b   | Modify blockers      | Toggle which tasks block the selected task               |
| m d   | Modify dependents    | Toggle which tasks the selected task blocks              |
...
| ~a s~ | Add successor (new task that selected task blocks)  |
| ~a b~ | Add blocker (new task that blocks selected task)    |
| ~m b~ | Modify blockers (toggle what blocks selected task)  |
| ~m d~ | Modify dependents (toggle what selected task blocks)|
...
- *Next step in sequence*: Select current task, ~a s~ (add successor)
...
*How to fix*: Use ~m b~ to modify blockers (uncheck incorrect ones), or use text command ~org-gtd-task-clear-relationships~ to start fresh.
...
- Press ~m b~ to modify its blockers
- Or use ~a s~ from an earlier task to add a successor that should come later
...
3. Project graph opens automatically:
   ...
   - Adjust dependencies with ~m b~ (modify blockers) or ~m d~ (modify dependents)
...
- Use ~m b~ to modify blockers, then ~R~ to recalculate
...
- Adding tasks with dependencies: ~a r~ (root), ~a s~ (successor), ~a b~ (blocker)
- Modifying relationships: ~m b~ (modify blockers), ~m d~ (modify dependents)
...
*How it's set*:
- Graph view: ~a s~ (add successor), ~a b~ (add blocker), ~m b~ (modify blockers)
...
2. *Visual inspection with graph view*:
   ...
   - Missing arrows? Use ~a s~ to add successor or ~m b~ to add blockers
   - Wrong arrows? Use ~m b~ or ~m d~ to uncheck incorrect relationships
   - Use ~i~ to inspect a task's current relationships

3. *Clear and rebuild*:
   If task relationships are completely corrupted:
   - In graph view: Select task, use ~m b~ and ~m d~ to adjust blockers/dependents
   - Or in text: Run ~M-x org-gtd-task-clear-relationships~
   - Rebuild using graph view: ~a s~ (add successor), ~a b~ (add blocker)
...
*Solution*:
- Fix circular dependencies: Use ~m b~ to remove incorrect blocker relationships
- Add root tasks: Use ~a r~ to create tasks that can start immediately
...
- Select a task, use ~m b~ to add blockers it should wait for
- Or use ~a s~ from an earlier task to add a successor that should come later
...
- If blocker shouldn't block this task: Use ~m b~ to modify blockers and uncheck it
- If many wrong dependencies: Run ~org-gtd-task-clear-relationships~, rebuild with ~a s~/~a b~
