...
- *Sensible defaults* - By default, all item types auto-refile to first target without prompting (set ~org-gtd-refile-prompt-for-types~ if you want to be prompted)
...
#+begin_src emacs-lisp
  ...
  ;; Optional: Enable per-type refile prompting (useful with multiple refile targets)
  ;; By default, all items auto-refile to first target without prompting
...
- ~org-gtd-refile-prompt-for-types~ :: list of GTD item types that should prompt for refile target selection. Types not in the list auto-refile to first target. Default is ~nil~ (all types auto-refile). [[#controlling-refile-prompts][Controlling Refile Prompts by Item Type]].
...
The ~org-gtd-refile-prompt-for-types~ variable controls which types prompt. It's a list of GTD item types that should prompt for target selection. By default, this is ~nil~ meaning all item types auto-refile to the first available target without prompting.

*Example: Prompt only for projects*

If you only care about choosing project locations:

#+begin_src elisp
(setq org-gtd-refile-prompt-for-types '(project-heading project-task))
#+end_src

*Example: Prompt for most types except quick actions*
...
*Type*: List of symbols or ~nil~

*Default*: ~nil~

*Description*: List of GTD item types that should prompt for refile target selection when organizing.

- When ~nil~ (default): all item types auto-refile to the first available target without prompting
- When a list: types in the list prompt user to choose from available refile targets; types not in the list auto-refile to first target
