...
- ~type~ :: Filter by GTD type: ~project~, ~next-action~, ~calendar~, ~incubated~, ~habit~
...
The most common filter is ~type~, which matches GTD item types: ~project~, ~next-action~, ~calendar~, ~delegated~, ~incubated~, ~habit~, ~stuck-project~, ~completed-project~.
...
- For ~incubated~: checks the incubation review date
...
*Matches*: Projects that were moved to someday/maybe

*Use case*: Review incubated projects during weekly review
...
- For ~incubated~ type: checks the incubation review date
...
- ~org-gtd-reflect-stuck-incubated-items~ :: Incubated items with invalid or missing review dates
