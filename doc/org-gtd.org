...
You learned sequential projects in [[#tutorial-your-first-project][Tutorial: Your First Project]]. This section covers advanced project features: parallel tasks, custom dependencies, and complex project structures.

*** Understanding Project Dependencies

Projects in org-gtd use a dependency graph to track task relationships. Version 4.0 introduced flexible dependencies - you're not limited to sequential execution anymore.

**** How dependencies work

Task dependencies are managed through two org properties:

- ~ORG_GTD_DEPENDS_ON~ :: Contains IDs of tasks that must complete before this task can start (what blocks this task)
- ~ORG_GTD_BLOCKS~ :: Contains IDs of tasks that this task blocks (what depends on this task)
- ~ORG_GTD_FIRST_TASKS~ :: (On project heading) Contains IDs of tasks with no dependencies (the starting points)

**** Sequential vs Parallel

- *Sequential* (default): Tasks execute one after another (A → B → C)
  - Only one task is NEXT at a time
  - Each task blocks the next
  - Great for workflows where order matters

- *Parallel*: Multiple tasks can be NEXT simultaneously
  - Multiple independent work streams
  - Tasks with no dependencies start together
  - Faster project completion

- *Mixed*: Some tasks parallel, others sequential
  - Most real projects use this
  - Example: Research and Planning happen in parallel, but both must finish before Building starts

**** The dependency graph (DAG)

Tasks form a Directed Acyclic Graph (DAG):
- *Directed*: Dependencies flow one way (A → B means B depends on A, not vice versa)
- *Acyclic*: No circular dependencies (A → B → C → A is prevented)
- *Graph*: Tasks can have multiple dependencies and block multiple tasks

org-gtd automatically prevents circular dependencies to avoid deadlocks.

*** Tutorial: Project with Parallel Tasks

Let's create a project where some tasks can happen simultaneously.
...
3. Make orthopedist appointment (independent research - can happen in parallel with 1 & 2)
4. Get diagnosis (depends on both appointments completing)

Tasks 1 and 3 can happen in parallel - they don't depend on each other.
...
**** Step 2: Organize with Custom Dependencies

After organizing as a project (~C-c c~ → ~[P]~), org-gtd creates sequential dependencies by default. We need to modify them for parallel execution.

**Open the project file:**

Find your project in the engage view (~C-c d e~), press ~RET~ on any task to jump to the project file.
...
:PROPERTIES:
:ORG_GTD: Projects
:ORG_GTD_FIRST_TASKS: <id-of-find-mri>
:END:
** NEXT Find place to get MRI
:PROPERTIES:
:ID: <id-of-find-mri>
:ORG_GTD: Actions
:ORG_GTD_BLOCKS: <id-of-make-mri-appt>
:END:
** TODO Make MRI appointment
:PROPERTIES:
:ID: <id-of-make-mri-appt>
:ORG_GTD: Actions
:ORG_GTD_DEPENDS_ON: <id-of-find-mri>
:ORG_GTD_BLOCKS: <id-of-make-ortho-appt>
:END:
** TODO Make orthopedist appointment
... (and so on, sequential)
#+end_example

**** Step 3: Modify for Parallel Execution

We want "Find MRI place" and "Make orthopedist appointment" to be parallel starting tasks.

**On the project heading**, add the orthopedist appointment ID to FIRST_TASKS:

#+begin_example
:ORG_GTD_FIRST_TASKS: <id-of-find-mri> <id-of-make-ortho-appt>
#+end_example

**On "Make orthopedist appointment" task**, remove the DEPENDS_ON property:

Remove the line: ~:ORG_GTD_DEPENDS_ON: <id-of-make-mri-appt>~

**On "Get diagnosis" task**, make it depend on BOTH appointments:

#+begin_example
:ORG_GTD_DEPENDS_ON: <id-of-make-mri-appt> <id-of-make-ortho-appt>
#+end_example

**Fix the BLOCKS properties:**
- Remove orthopedist appointment from MRI appointment's BLOCKS
- Add diagnosis ID to both appointment tasks' BLOCKS

**** Step 4: Fix Keywords

Put point on the project heading and run:

~M-x org-gtd-projects-fix-todo-keywords-for-project-at-point~

This recalculates which tasks should be NEXT based on the dependency graph.

**** Step 5: See the Results

~C-c d e~ to view the engage agenda.

You'll now see:
- *NEXT: Find place to get MRI*
- *NEXT: Make orthopedist appointment*

Both are NEXT simultaneously! You can work on whichever you want.

When you complete both appointments, "Get diagnosis" automatically becomes NEXT.

**** The Dependency Pattern (Diamond)

This is called a "diamond" pattern:
...
Two parallel paths converge at a single task. This is common in real projects.

*** Tutorial: Complex Project Dependencies

Let's look at a more complex example: "Release Next Version of Software".
...
1. *Parallel branches* after List Features
2. *Converging paths* at Get Beta Feedback (both branches must complete)
3. *Parallel updates* after feedback
4. *Final convergence* at Release

**** Setting This Up

You would:
1. Capture the project structure
2. Organize as project (creates sequential by default)
3. Open the project file
4. Modify FIRST_TASKS to include only "List Features"
5. Set DEPENDS_ON for "Implement" to only List Features
6. Set DEPENDS_ON for "Write Documentation" to only List Features
7. Set DEPENDS_ON for "Get Beta Feedback" to both Implement AND Write Documentation
8. Set DEPENDS_ON for both Update tasks to Get Beta Feedback
9. Set DEPENDS_ON for "Release" to both Update tasks
10. Run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~

*** Common Dependency Patterns

**** Sequential Chain (A → B → C)
The simplest pattern. Default for all projects.

Use when: Order matters, each task depends on the previous.

Example: Review blueprints → Order windows → Schedule carpenter

**** Parallel Independent (A, B, C all start together)
All tasks are in FIRST_TASKS, no dependencies between them.

Use when: Tasks are completely independent and can all start immediately.

Example: Research destinations, Check budget, Ask for time off (for vacation planning)

**** Fan-out (A → B, A → C, A → D)
One task blocks multiple tasks that can then proceed in parallel.

Use when: Initial work enables multiple independent paths.

Example: Get project approval → (Design UI, Write backend API, Setup infrastructure)

**** Fan-in / Convergence (A → C, B → C)
Multiple tasks must complete before next task can start. Also called "join".

Use when: Final task needs all previous work completed.

Example: (Write code, Write tests) → Merge to main

**** Diamond (A → B → D, A → C → D)
Parallel paths that converge. Combination of fan-out and fan-in.

Use when: Work splits, proceeds in parallel, then combines.
...
- You're not sure what the next task will be until you complete the current one
- The project is simple and you don't want to think about dependencies
...
- Clear what all tasks are upfront

**** Managing complexity
- Start sequential, make parallel only when needed
- Use areas of focus or tags to group related tasks
- Keep task names clear and specific
- Review stuck projects weekly to catch dependency issues

**** Modifying projects over time
- Add tasks: Use "Add to existing project" when organizing new items
- Change dependencies: Edit properties directly, then run fix-keywords command
- Split tasks: Break down a task into sub-tasks, adjust dependencies
- Merge tasks: Mark unnecessary tasks DONE or CNCL

*** Reference: Dependency Management Commands

**** During organizing
- ~org-gtd-organize~ → ~[P]~ :: Creates project with sequential dependencies (default)
- ~org-gtd-organize~ → ~[A]~ :: Adds task to existing project

**** After organizing
- ~org-gtd-projects-fix-todo-keywords-for-project-at-point~ :: Recalculates NEXT tasks based on dependency graph. Use after manually editing dependencies.

**** Properties to edit manually
- ~ORG_GTD_FIRST_TASKS~ :: (on project heading) Space-separated list of task IDs with no dependencies
- ~ORG_GTD_DEPENDS_ON~ :: (on task) Space-separated list of task IDs this task depends on
- ~ORG_GTD_BLOCKS~ :: (on task) Space-separated list of task IDs this task blocks

After editing these properties, always run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~ to update task states.

*** Optional: Visualizing Dependencies

Enable visual dependency helper while clarifying projects:

#+begin_src emacs-lisp
(setq org-gtd-clarify-display-helper-buffer t)
#+end_src

When enabled, a side window shows task relationships as you work on a project in the WIP buffer during clarification.
...
- Missing FIRST_TASKS (project has no starting point)
- All first tasks are DONE but dependencies not set up correctly

Fix: Put point on project heading, run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~

**** Task should be NEXT but shows as TODO

The dependency graph says it's blocked. Check:
1. What tasks are in its ORG_GTD_DEPENDS_ON?
2. Are those tasks actually DONE?
3. Did you forget to remove a dependency?

Fix: Edit ORG_GTD_DEPENDS_ON property, run fix-keywords command.

**** Too many tasks are NEXT

You probably have tasks that should have dependencies but don't.

Fix: Add appropriate ORG_GTD_DEPENDS_ON properties to tasks that shouldn't start yet, run fix-keywords command.

*See also*:
- [[#properties-reference][Properties Reference]] for complete documentation of ~ORG_GTD_DEPENDS_ON~, ~ORG_GTD_BLOCKS~, and ~ORG_GTD_FIRST_TASKS~ properties
- [[#commands-reference][Commands Reference]] for all project management and task dependency commands
...
*** Oops View Configuration

**** ~org-gtd-oops-custom-views~

*Type*: List of GTD view specifications

*Default*: ~nil~

*Description*: Additional custom "oops" views for finding missed/overdue items.

Each view is a GTD view specification alist with ~'name~ and ~'filters~ keys.

*Example*:
#+begin_src emacs-lisp
(setq org-gtd-oops-custom-views
      '(((name . "Overdue Work Items")
         (filters . ((area-of-focus . "Work")
                     (deadline . past))))
        ((name . "Missed Delegations")
         (filters . ((category . delegated)
                     (timestamp . past))))))
#+end_src

Use ~org-gtd-oops-with-custom~ to see all oops views including your custom ones.
...
See source code for examples of the expected structure.
...
- ~RET~ :: Visit item
- ~r~ :: Refile
- And all other org-agenda commands
...
*** Oops Commands (Missed/Overdue Items)

**** ~org-gtd-oops~

*When to use*: Daily or weekly to catch missed appointments and overdue tasks

*What it does*:
- Shows all missed calendar appointments
- Shows overdue projects
- Shows past-due delegations

**** ~org-gtd-oops-calendar~

Shows only missed calendar appointments.

**** ~org-gtd-oops-delegated~

Shows only delegated items where the follow-up date has passed.

**** ~org-gtd-oops-projects~

Shows only projects with overdue deadlines.

**** ~org-gtd-oops-with-custom~

Shows all oops views plus any custom views defined in ~org-gtd-oops-custom-views~.
...
*When to use*: After manually editing project dependencies
...
*Example workflow*:
1. Edit ORG_GTD_DEPENDS_ON properties manually
2. Put point on project heading
3. Run this command
4. Keywords are updated to match dependencies

*** Task Dependency Commands

These commands help manage task relationships in projects:
...
org-gtd uses org-mode properties extensively to track state and relationships. This section documents all properties used by the system.
...
*When to modify*:
- After organizing a project (org-gtd sets this automatically)
- When adding parallel starting points to a project
- Always run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~ after manual edits
...
*When to modify*:
- When creating custom dependency structures
- When changing task relationships
- Always update ORG_GTD_BLOCKS on the blocking tasks too (bidirectional)
- Always run ~org-gtd-projects-fix-todo-keywords-for-project-at-point~ after edits
...
*When to modify*:
- Manually when setting up dependencies (maintain bidirectional relationship)
- Use ~org-gtd-task-add-dependent~ command instead of manual edits when possible
...
*When to modify*: Use ~org-gtd-organize~ to change calendar items rather than manual edits.
...
*When to modify*: Use ~org-gtd-delegate-agenda-item~ or organize flow rather than manual edits.
...
org-gtd provides hooks to customize behavior when organizing items. This section documents the hooks framework and how to write custom hooks.
...
*Solution*: Put the point on the project heading and run ~M-x org-gtd-projects-fix-todo-keywords-for-project-at-point~.
