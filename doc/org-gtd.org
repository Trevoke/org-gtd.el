...
- ~filters~ :: What to include.
  - ~type . project~ :: Only show project headings (not single actions)
...
- ~type~ :: Filter by GTD type: ~project~, ~next-action~, ~calendar~, ~tickler~, ~someday~, ~habit~
...
Filter by CLOSED timestamp.

Values: ~today~, ~recent~ (7 days), ~past-day~, ~past-week~, ~past-month~, ~past-year~
...
(defun my-weekly-review ()
  "Weekly review: stuck projects, missed items, recent completions."
  (interactive)
  (org-gtd-view-show
   '(((name . "Stuck Projects")
      (type . stuck-project))
...
(defun my-morning-view ()
  "Morning planning: calendar, delegated check-ins, NEXT actions."
  (interactive)
  (org-gtd-view-show
   '((name . "Morning Planning")
     (blocks . (((name . "Today's Schedule")
                 (block-type . calendar-day))
                ((name . "Delegated Items to Check")
                 (type . delegated)
                 (when . today))
