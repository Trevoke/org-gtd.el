...
1. While clarifying any item, press ~d~
...
|-----+---------+----------|
| ~d~ | ~org-gtd-clarify-duplicate~ | Prompts for new title (current title as default) |
| ~D~ | ~org-gtd-clarify-duplicate-exact~ | Copies exactly, no prompt |

Use ~d~ when the duplicates have different titles (most common). Use ~D~ when you want identical copies to organize differently (e.g., same task delegated to multiple people).
