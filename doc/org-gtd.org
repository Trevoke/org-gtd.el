...
:END:
The clarify buffers use the major mode ~org-gtd-clarify-mode~, which is derived from ~org-mode~. There is a keymap, ~org-gtd-clarify-mode-map~, that you can use to bind useful
...
If you're unsure which GTD category to choose when organizing, the *organize help buffer* shows a quick reference of all item types with their keyboard shortcuts and key properties:
- ~org-gtd-clarify-toggle-organize-help~ :: toggles a side window showing all GTD item types (Quick Action, Single Action, Project, Calendar, etc.) with brief descriptions
- ~org-gtd-clarify-show-organize-help~ :: set to ~'right~, ~'left~, ~'top~, or ~'bottom~ to configure the window position
...
When you organize an item as a Project, org-gtd finds *all* headings with ~ORG_GTD_REFILE: Projects~ (plus any targets from your ~org-refile-targets~) and lets you choose where to refile.
