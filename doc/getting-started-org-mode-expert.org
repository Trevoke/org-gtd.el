...
| ~org-gtd-tasks.org~ | All organized items |

All GTD categories are stored as top-level headings within ~org-gtd-tasks.org~:
- ~* Actions~ - Single actions
- ~* Projects~ - Multi-step projects
- ~* Calendar~ - Time-specific items
- ~* Delegated~ - Waiting for others
- ~* Tickler~ - Remind on date
- ~* Someday~ - No timeframe

These headings are created automatically on first refile. The files are standard org files with no magic.
